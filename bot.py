...
# Scoring: done=3, postpone=1, skip=0
SCORE = {"done": 3, "postpone": 1, "skip": 0}
...
<title>KuroTasker Dashboard</title>
...
:root{
  --bg:#07090e;--s1:#0d1018;--s2:#141820;--bd:#1c2130;
  --done:#4ade80;--skip:#facc15;--post:#f97316;
  --txt:#dde3f0;--mut:#4a5568;--acc:#818cf8;
}
*{margin:0;padding:0;box-sizing:border-box;}
body{background:var(--bg);color:var(--txt);font-family:'DM Mono',monospace;min-height:100vh;}
body::before{content:'';position:fixed;top:-25%;left:-15%;width:65%;height:65%;
  background:radial-gradient(ellipse,rgba(99,102,241,.08) 0%,transparent 65%);pointer-events:none;z-index:0;}
body::after{content:'';position:fixed;bottom:-20%;right:-10%;width:55%;height:55%;
  background:radial-gradient(ellipse,rgba(74,222,128,.06) 0%,transparent 65%);pointer-events:none;z-index:0;}

header{position:relative;z-index:1;padding:2.5rem 3rem 2rem;border-bottom:1px solid var(--bd);
...
.btn{background:var(--s2);border:1px solid var(--bd);color:var(--txt);font-family:'DM Mono',monospace;
  font-size:.68rem;padding:.5rem 1.2rem;cursor:pointer;letter-spacing:.08em;text-transform:uppercase;transition:all .2s;}
...
.card.c-acc::before{background:var(--acc);}
.card.c-done::before{background:var(--done);}
.card.c-skip::before{background:var(--skip);}
.card.c-post::before{background:var(--post);}
...
.card.c-acc  .cval{color:var(--acc);}
.card.c-done .cval{color:var(--done);}
.card.c-skip .cval{color:var(--skip);}
.card.c-post .cval{color:var(--post);}
.csub{font-size:.62rem;color:var(--mut);margin-top:.3rem;}

/* ── section title ── */
.stitle{font-size:.6rem;letter-spacing:.15em;text-transform:uppercase;color:var(--mut);
  margin-bottom:1rem;padding-bottom:.5rem;border-bottom:1px solid var(--bd);
...
.pd{background:rgba(74,222,128,.15);color:var(--done);}
.ps{background:rgba(250,204,21,.15);color:var(--skip);}
.pp{background:rgba(249,115,22,.15);color:var(--post);}
...
.empty p{font-size:.74rem;line-height:1.9;}
.empty code{background:var(--s2);padding:.1rem .4rem;color:var(--acc);}
...
<script>
const C = {
  bg:'#07090e', s1:'#0d1018', bd:'#1c2130',
  done:'rgba(74,222,128,.85)', skip:'rgba(250,204,21,.75)', post:'rgba(249,115,22,.75)',
  acc:'rgba(129,140,248,.85)', txt:'#94a3b8', grid:'#1c2130'
};
...
function render(d){
  ...
  if(!overall.length){
    dash.innerHTML=`<div class="empty"><h2>No data yet</h2><p>Add tasks and start responding to reminders on Telegram.<br>Then refresh this page.</p></div>`;
  ...
  const totalScore=overall.reduce((s,o)=>s+o.done*3+o.postpone*1,0);
  const maxScore=tot*3;
  const scorePct=maxScore?Math.round(totalScore/maxScore*100):0;

  document.getElementById('sub').textContent=
    `${overall.length} task${overall.length!==1?'s':''} · last updated ${new Date(d.generated_at).toLocaleTimeString()}`;
  ...
  dash.innerHTML=`
    ...
    <div class="stitle"><span></span> Score Over Time (per task)</div>
    <div class="chart-full">
      <div class="cbox-title">Daily score: Done=3pts · Postponed=1pt · Skipped=0pts</div>
      <canvas id="score-chart"></canvas>
      <div class="score-legend">
        ${overall.map((o,i)=>`<span><div class="dot" style="background:${lineColor(i)}"></div>${o.task}</span>`).join('')}
      </div>
    </div>
    ...
    <div class="crow">
      <div class="cbox">
        <div class="cbox-title">Daily breakdown (all tasks combined)</div>
        <canvas id="bar-chart"></canvas>
      </div>
      <div class="cbox">
        <div class="cbox-title">Overall distribution</div>
    ...
    <table>
      <thead><tr><th>Task</th><th>Done</th><th>Skipped</th><th>Postponed</th><th>Score</th><th>Completion</th></tr></thead>
      <tbody>${overall.map(o=>{
        const t=o.done+o.skip+o.postpone;
        const pct=t?Math.round(o.done/t*100):0;
        const sc=o.done*3+o.postpone*1;
        const maxSc=t*3;
        const spct=maxSc?Math.round(sc/maxSc*100):0;
        const col=pct>=70?'var(--done)':pct>=40?'var(--skip)':'var(--post)';
        return `<tr>
          <td><div class="tname">${o.task}</div></td>
          <td><span class="pill pd">✓ ${o.done}</span></td>
          <td><span class="pill ps">⏭ ${o.skip}</span></td>
          <td><span class="pill pp">⏰ ${o.postpone}</span></td>
          <td><span class="rate" style="color:var(--acc)">${spct}%</span><div class="csub">${sc}/${maxSc}pts</div></td>
  ...
  // ─── Score line chart (per task) ───
  const firstTask = Object.keys(daily)[0];
  if(firstTask){
    const labels = daily[firstTask].dates.map(d=>{
      const dt=new Date(d); return `${dt.getMonth()+1}/${dt.getDate()}`;
    });

    // per-task daily score
    const scoreDatasets = overall.map((o,i)=>{
      const td2 = daily[o.task];
      if(!td2) return null;
      return {
        label: o.task,
        data: td2.dates.map((_,idx)=> td2.done[idx]*3 + td2.postpone[idx]*1),
        borderColor: lineColor(i),
        backgroundColor: lineColor(i).replace('1)','0.08)'),
        borderWidth: 2,
        pointRadius: 3,
        pointHoverRadius: 5,
        tension: 0.4,
    ...
    charts.score = new Chart(document.getElementById('score-chart').getContext('2d'),{
      type:'line',
      ...
      options:{
        ...
        scales:{
          x:{ticks:{color:C.txt,font,maxTicksLimit:12},grid:{color:C.bd}},
          y:{ticks:{color:C.txt,font},grid:{color:C.bd},beginAtZero:true,
             title:{display:true,text:'Score (pts)',color:C.txt,font}}
        }
      }
    });

    // ─── Stacked bar ───
    const doneD  = labels.map((_,i)=>Object.values(daily).reduce((s,t)=>s+(t.done[i]||0),0));
    const skipD  = labels.map((_,i)=>Object.values(daily).reduce((s,t)=>s+(t.skip[i]||0),0));
    const postD  = labels.map((_,i)=>Object.values(daily).reduce((s,t)=>s+(t.postpone[i]||0),0));

    charts.bar = new Chart(document.getElementById('bar-chart').getContext('2d'),{
      type:'bar',
      data:{labels,datasets:[
        {label:'Done',     data:doneD, backgroundColor:C.done, borderRadius:2},
        {label:'Skip',     data:skipD, backgroundColor:C.skip, borderRadius:2},
        {label:'Postpone', data:postD, backgroundColor:C.post, borderRadius:2},
      ]},
      options:{
        responsive:true,maintainAspectRatio:true,
        plugins:{legend:{labels:{color:C.txt,font,boxWidth:10}}},
        scales:{
          x:{stacked:true,ticks:{color:C.txt,font,maxTicksLimit:10},grid:{color:C.bd}},
          y:{stacked:true,ticks:{color:C.txt,font},grid:{color:C.bd}}
        }
      }
    });
    ...
    charts.donut = new Chart(document.getElementById('donut-chart').getContext('2d'),{
      type:'doughnut',
      data:{
        labels:['Done','Skipped','Postponed'],
        datasets:[{data:[td,ts,tp],
          backgroundColor:['rgba(74,222,128,.8)','rgba(250,204,21,.7)','rgba(249,115,22,.7)'],
          borderColor:'#07090e',borderWidth:3,hoverOffset:8}]
      },
      options:{
        responsive:true,maintainAspectRatio:true,cutout:'70%',
        plugins:{legend:{position:'bottom',labels:{color:C.txt,font,padding:14,boxWidth:10}}}
      }
    });
  }
}
...
function lineColor(i){
  const cols=[
    'rgba(129,140,248,1)','rgba(74,222,128,1)','rgba(249,115,22,1)',
    'rgba(250,204,21,1)','rgba(236,72,153,1)','rgba(34,211,238,1)'
  ];
...
def api_stats():
    ...
    tasks = list(dict.fromkeys(r["task_name"] for r in rows))  # preserve order
    ...
    for task in tasks:
        ...
        daily[task] = {
            "dates":    dates,
            "done":     [d[x]["done"]     for x in dates],
            "skip":     [d[x]["skip"]     for x in dates],
            "postpone": [d[x]["postpone"] for x in dates],
        }
    ...
    overall = [
        {"task": t, "done": totals[t]["done"], "skip": totals[t]["skip"], "postpone": totals[t]["postpone"]}
        for t in tasks
    ]
    return jsonify({"overall": overall, "daily": daily, "generated_at": datetime.now(IST).isoformat()})
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Bot activated! Chat ID saved: {chat_id}\n\n"
        "Commands:\n"
        "/addtask Name | HH:MM  — daily task\n"
        "/addweekly Name | Mon,Tue,Wed | HH:MM  — weekly\n"
...
async def remove_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        conn.commit()
        conn.close()
        await update.message.reply_text(f"🗑️ Task {task_id} removed.")
    except Exception:
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for task, c in summary.items():
        ...
        score = c["done"]*3 + c["postpone"]*1
        msg  += f"{task}\n  ✅ {c['done']}  ⏭️ {c['skip']}  ⏰ {c['postpone']}  |  Score: {score}pts  ({pct}%)\n\n"
    await update.message.reply_text(msg)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"{emoji[data['status']]} {data['name']} — marked as {data['status']}"
    )
    ...
    for t in get_tasks():
        ...
        if key not in sent and should_send_now(t["schedule"]):
            ...
            kb = InlineKeyboardMarkup([[
                ...
                InlineKeyboardButton("⏭️ Skip",     callback_data=json.dumps({"id": t["id"], "name": t["name"], "status": "skip",     "at": at})),
